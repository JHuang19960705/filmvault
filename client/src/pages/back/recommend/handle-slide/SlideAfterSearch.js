    ...
    <tbody className="text-gray-600 dark:text-gray-100">
      <tr className="hidden md:table-row">
        <td className="sm:p-3 py-2 px-1 w-1/4 border-b border-gray-200 dark:border-gray-600">
          <div className="flex items-center"><p className="w-[140px] truncate">{data && data.original_name || data.title}</p></div>
        </td>
        <td className="sm:p-3 py-2 px-1 w-1/4 border-b border-gray-200 dark:border-gray-600">
          <div className="flex items-center">
            <a className="imageContainer">
              <Link to={`/movie/${data.id}`} className="imageContainer" target="_blank">
                <img className="w-20" src={TMDB_IMG_SM + data.poster_path} loading="lazy" decoding="async" />
              </Link>
            </a>
          </div>
        </td>
      ...
      <div className="mb-5 flex justify-center md:justify-around items-center border-b border-grey-dark pb-5 md:hidden">
        ...
        <div className="pl-4">
          <div className="w-[150px] mt-2 text-base font-bold text-secondary truncate ">{data && data.original_name || data.title}</div>
