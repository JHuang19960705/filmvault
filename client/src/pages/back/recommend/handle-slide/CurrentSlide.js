...
export default function CurrentSlide() {
  ...
  const settings = {
    dots: false,
    infinite: currentUser.user.slide.tmdbImgBackdrop.length >= 2,
    speed: 2500,
    slidesToShow: 1,
    slidesToScroll: 1,
    arrows: false,
  ...
  if (currentUser && currentUser.user.slide.tmdbImgBackdrop) {
    backdropElements = currentUser.user.slide.tmdbImgBackdrop.map((backdrop, index) => {
      return (
        <div key={index}>
  ...
  return (
    <div id="carousel-wrap">
      ...
      {currentUser.user.slide.tmdbImgBackdrop.length >= 2 &&
        <div id="controls">
