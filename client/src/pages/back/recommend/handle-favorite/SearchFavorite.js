                ...
                if (d.name && d.profile_path && d.known_for_department) {
                  return (
                    <tr className="hidden md:table-row" key={d.id}>
                      ...
                      <td className="sm:p-3 py-2 px-1 w-1/4 border-b border-gray-200 dark:border-gray-800">
                        <div className="flex items-center">
                          <button onClick={() => { handleChangeOpen2(); setNewFavorite({ id: d.id, name: d.name }) }} data-tmdb-id={d.id} data-tmdb-img={d.profile_path} className="mt-5 heading rounded-md bg-blue-500 px-3 py-1 text-white">
                            選取slide
                          </button>
                        </div>
                      </td>
