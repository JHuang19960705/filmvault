...
export default function CurrentTheater() {
  ...
  useEffect(() => {
    if(currentUser.user.theater.releases.tmdbImgBackdrop) {
      setLoading(false)
    }
  }, [currentUser]);
  ...
  if (currentUser && currentUser.user.theater.releases.tmdbImgBackdrop) {
    backdropElements = currentUser.user.theater.releases.tmdbImgBackdrop.map((backdrop, index) => {
      return (
        <div key={index}>
